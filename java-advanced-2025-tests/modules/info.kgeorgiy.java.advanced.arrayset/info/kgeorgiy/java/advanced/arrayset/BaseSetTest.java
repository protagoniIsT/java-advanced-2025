...
public abstract class BaseSetTest extends BaseTest {
    ...
    protected interface TestCase<E, T> {
        void test(SetPair<E, T> pair);
    }

    protected interface TestPair<E, T> {
        void test(SetPair<E, T> pair, E first, E second);
    }
    ...
    protected <S extends Collection<Integer>> SetPair<Integer, S> pair(
            ...
            final NamedComparator<Integer> comparator
    ) {
        return createSets(elements, comparator, this::values);
    }
    ...
    protected static <E, S extends Collection<E>> SetPair<E, S> createSets(
            final List<E> elements,
            final Comparator<E> comparator,
            final BiFunction<Comparator<E>, Collection<E>, Collection<E>> values
    ) {
        return new SetPair<>(
                elements,
                comparator,
                ...
                "[comparator = %s, elements = %s]".formatted(comparator, elements),
                values
        );
    }
    ...
    protected static class SetPair<E, S> {
        protected final List<E> elements;
        final Comparator<E> comparator;
        final S model;
        final S tested;
        final String context;
        private final BiFunction<Comparator<E>, Collection<E>, Collection<E>> values;
        ...
        protected <R, V> void testGet(
            ...
            Assertions.assertEquals(expected, actual, () -> format.formatted(value) + " " + context);
            if (expected instanceof SortedSet) {
                Assertions.assertEquals(
                        ((SortedSet<?>) expected).comparator(),
                        ((SortedSet<?>) actual).comparator(),
                        () -> format.formatted(value) + " " + context + " .comparator()"
                );
            }
        }
        ...
        public <R> SetPair<E, R> transform(final Function<S, R> f) {
            return transform(f, f);
        }

        public <R> SetPair<E, R> transform(final Function<S, R> m, final Function<S, R> t) {
