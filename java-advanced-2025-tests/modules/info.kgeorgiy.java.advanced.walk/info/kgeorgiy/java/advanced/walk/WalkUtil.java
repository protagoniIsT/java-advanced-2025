...
public class WalkUtil {
    ...
    private final Random random = new Random(2750287502375224500L);
    /* package-private */ List<String> alphabet = ENGLISH_DIGITS;
    /* package-private */ final String errorHash;
    ...
    public String hash(final byte[] bytes) {
        ...
        try {
            ...
            final byte[] hash = bytes == null ? new byte[8] : Arrays.copyOf(digest.digest(bytes), 8);
            return ("%0" + (hash.length << 1) + "x").formatted(new BigInteger(1, hash));
